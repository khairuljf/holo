...
add_action( 'after_setup_theme', 'squadrone_register_menus' );
add_action( 'wp_enqueue_scripts', 'squadrone_enqueue_scripts_styles' );
add_action( 'tgmpa_register', 'squadrone_register_plugins' );
add_action( 'wp_enqueue_scripts', 'squadrone_load_fonts' );

add_action( 'admin_enqueue_scripts', 'squadrone_load_admin_style' );

add_filter( 'bt_bb_color_scheme_arr', 'squadrone_color_schemes' );

add_filter( 'body_class', 'squadrone_body_class' );
 ...
 */
if ( ! function_exists( 'squadrone_register_menus' ) ) {
	function squadrone_register_menus() {
		register_nav_menus( array (
			'primary' => esc_html__( 'Primary Menu', 'squadrone' ),
			'footer'  => esc_html__( 'Footer Menu', 'squadrone' )
		));
	}
}
 ...
 */
if ( ! function_exists( 'squadrone_enqueue_scripts_styles' ) ) {
	function squadrone_enqueue_scripts_styles() {
		...
		// custom theme css
		wp_enqueue_style( 'squadrone-core', get_template_directory_uri() . '/css/core.css', array(), false, 'screen' );
        wp_enqueue_style( 'squadrone-style', get_template_directory_uri() . '/style.css', array(), false, 'screen' );
		wp_enqueue_style( 'squadrone-print', get_template_directory_uri() . '/print.css', array(), false, 'print' );
		...
		wp_enqueue_script( 'squadrone-modernizr', get_template_directory_uri() . '/framework/js/modernizr.custom.js', array( 'jquery' ), '', false );

		// custom theme js
		wp_enqueue_script( 'squadrone-header', get_template_directory_uri() . '/framework/js/header.misc.js', array( 'jquery' ), '', true );
		wp_enqueue_script( 'squadrone-misc', get_template_directory_uri() . '/framework/js/misc.js', array( 'jquery' ), '', true );
		
		wp_add_inline_script( 'squadrone-header', boldthemes_set_global_uri(), 'before' );
		...
		if ( file_exists( get_template_directory() . '/css-override.php' ) && $boldthemes_add_override_css ) {
			...
			wp_add_inline_style( 'squadrone-style', $css_override );
		}
		
		if ( file_exists( get_template_directory() . '/icons.php' ) ) {
			require_once( get_template_directory() . '/icons.php' );
			wp_add_inline_style( 'squadrone-style', $icons );
		}

		if ( boldthemes_get_option( 'custom_js' ) != '' ) {
			wp_add_inline_script( 'squadrone-modernizr', boldthemes_get_option( 'custom_js' ) );
		}	
 ...
 */
if ( ! function_exists( 'squadrone_register_plugins' ) ) {
	function squadrone_register_plugins() {

		$plugins = array(
	 
			array(
				'name'               => esc_html__( 'Squadrone', 'squadrone' ), // The plugin name.
				'slug'               => 'squadrone', // The plugin slug (typically the folder name).
				'source'             => get_template_directory() . '/plugins/squadrone.zip', // The plugin source.
			...
			array(
				'name'               => esc_html__( 'Cost Calculator', 'squadrone' ), // The plugin name.
			...
			array(
				'name'               => esc_html__( 'Bold Builder', 'squadrone' ), // The plugin name.
			...
			array(
				'name'               => esc_html__( 'BoldThemes WordPress Importer', 'squadrone' ), // The plugin name.
				...
				'required'           => true, // If false, the plugin is only 'recommended' instead of required.
				'version'            => '2.1.0', // E.g. 1.0.0. If set, the active plugin must be this version or higher.
				'force_activation'   => false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch.
				'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins.
				'external_url'       => '', // If set, overrides default API URL and points to an external URL.
			),
			array(
				'name'               => esc_html__( 'Meta Box', 'squadrone' ), // The plugin name.
			...
			array(
				'name'               => esc_html__( 'Contact Form 7', 'squadrone' ), // The plugin name.
			...
			array(
				'name'               => esc_html__( 'WooSidebars', 'squadrone' ), // The plugin name.
		...
		$config = array(
			...
			'strings'      => array(
				...
				'installing'                      => esc_html__( 'Installing Plugin: %s', 'squadrone' ), // %s = plugin name.
				'oops'                            => esc_html__( 'Something went wrong with the plugin API.', 'squadrone' ),
				'notice_can_install_required'     => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'squadrone' ), // %1$s = plugin name(s).
				'notice_can_install_recommended'  => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'squadrone' ), // %1$s = plugin name(s).
				'notice_cannot_install'           => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'squadrone' ), // %1$s = plugin name(s).
				'notice_can_activate_required'    => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'squadrone' ), // %1$s = plugin name(s).
				'notice_can_activate_recommended' => _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'squadrone' ), // %1$s = plugin name(s).
				'notice_cannot_activate'          => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'squadrone' ), // %1$s = plugin name(s).
				'notice_ask_to_update'            => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'squadrone' ), // %1$s = plugin name(s).
				'notice_cannot_update'            => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'squadrone' ), // %1$s = plugin name(s).
				'install_link'                    => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'squadrone' ),
				'activate_link'                   => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'squadrone' ),
				...
				'complete'                        => esc_html__( 'All plugins installed and activated successfully. %s', 'squadrone' ), // %s = dashboard link.
 ...
 */
if ( ! function_exists( 'squadrone_load_fonts' ) ) {
	function squadrone_load_fonts() {
		$body_font = urldecode( boldthemes_get_option( 'body_font' ) );
		$heading_font = urldecode( boldthemes_get_option( 'heading_font' ) );
		$menu_font = urldecode( boldthemes_get_option( 'menu_font' ) );
		$heading_subtitle_font = urldecode( boldthemes_get_option( 'heading_subtitle_font' ) );
		...
		if ( $body_font != '' ) {
			...
			$body_font_state = _x( 'on', 'Lato font: on or off', 'squadrone' );
		...
		if ( $heading_font != '' ) {
			...
			$heading_font_state = _x( 'on', 'Lato font: on or off', 'squadrone' );
		...
		if ( $menu_font != '' ) {
			...
			$menu_font_state = _x( 'on', 'Lato font: on or off', 'squadrone' );
		...
		if ( $heading_subtitle_font != '' ) {
			...
			$heading_subtitle_font_state = _x( 'on', 'Lato font: on or off', 'squadrone' );
		...
		if ( $heading_supertitle_font != '' ) {
			...
			$heading_supertitle_font_state = _x( 'on', 'Lato font: on or off', 'squadrone' );
...
if ( ! function_exists( 'squadrone_load_admin_style' ) ) {
	function squadrone_load_admin_style() {
		wp_enqueue_style( 'squadrone-style', get_template_directory_uri() . '/admin-style.css' );
		require_once( get_template_directory() . '/admin-style.php' );
		wp_add_inline_style( 'squadrone-style', $admin_style );
	}
}
 ...
 * @return string 
 */
if ( ! function_exists( 'squadrone_body_class' ) ) {
	function squadrone_body_class( $extra_class ) {
