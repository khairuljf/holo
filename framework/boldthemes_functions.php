	...
	function boldthemes_get_new_media_html( $arg = array() ) { 
		...
		if ( $video != '' ) {
			...
			if ( strpos( $video, 'vimeo.com/' ) > 0 ) {
				...
				$html .= '<ifra' . 'me src="' . esc_attr( 'https://player.vimeo.com/video/' . $video_id ) . '" allowfullscreen></ifra' . 'me>';
			} else {
			...
			$html = '<div class="btMediaBox btDarkSkin btLink"' . $bt_link_bg_image . '><blockquote><p><a href="' . esc_attr( $link_url ) . '">' . wp_kses_post( $link_title ) . '</a></p><cite><a href="' . esc_attr( $link_url ) . '">' . wp_kses_post( $link_url ) . '</a></cite></blockquote></div>';
	
		}  else if ( $quote != '' ) {
			$bt_quote_bg_image = "";
			if ( $featured_image != '' ) $bt_quote_bg_image .= " style='background-image: url(" . esc_url_raw( $featured_image ) . ")'";
			$html = wp_kses_post( $quote );
			if ( $permalink != '' ) $html = '<a href="' . $permalink . '" alt="' . $title . '" title="' . $title . '">' . $html .'</a>';
			...
			if ( $share_facebook ) {
				$share_html .= boldthemes_get_icon_html( 
					array ( 
						...
						'url_title' => esc_html__( 'Share on Facebook', 'squadrone' ), 
						'style' => $style,
						'shape' => $shape,
						'size' => $size,
			...
			if ( $share_twitter ) {
				$share_html .= boldthemes_get_icon_html( 
					array ( 
						...
						'url_title' => esc_html__( 'Share on Twitter', 'squadrone' ), 
						'style' => $style,
						'shape' => $shape,
						'size' => $size,
						'color_scheme' => $color_scheme, 
			...
			if ( $share_linkedin ) {
				$share_html .= boldthemes_get_icon_html( 
					array ( 
						...
						'url_title' => esc_html__( 'Share on Linkedin', 'squadrone' ), 
						'style' => $style,
						'shape' => $shape,
						'size' => $size, 
						'color_scheme' => $color_scheme,
			...
			if ( $share_google_plus ) {
				$share_html .= boldthemes_get_icon_html( 
					array ( 
						...
						'url_title' => esc_html__( 'Share on Google Plus', 'squadrone' ), 
						'style' => $style,
						'shape' => $shape,
						'size' => $size, 
						'color_scheme' => $color_scheme, 
			...
			if ( $share_vk ) {
				$share_html .= boldthemes_get_icon_html( 
					array ( 
						...
						'url_title' => esc_html__( 'Share on VK', 'squadrone' ), 
						'style' => $style,
						'shape' => $shape,
						'size' => $size,  
						'color_scheme' => $color_scheme,
...
if ( ! function_exists( 'boldthemes_logo' ) ) {
	function boldthemes_logo( $type = 'header' ) {
		...
		if ( $logo != '' && $logo != ' ' ) {
			if ( $type == 'header' ) {
				echo '<a href="' . esc_attr( $home_link ) . '"><img class="btMainLogo" data-hw="' . esc_attr( $hw ) . '" src="' . esc_attr( $logo ) . '" alt="' . esc_attr( get_bloginfo( 'name' ) ) . '">';
				if ( $alt_logo != '' && $alt_logo != ' ' ) echo '<img class="btAltLogo" src="' . esc_attr( $alt_logo ) . '" alt="' . esc_attr( get_bloginfo( 'name' ) ) . '">';
				echo '</a>';
			} else if ( $type == 'footer' ) {
				echo '<a href="' . esc_attr( $home_link ) . '"><img src="' . esc_attr( $logo ) . '" alt="' . esc_attr( get_bloginfo( 'name' ) ) . '"></a>';
			...
			echo '<a href="' . esc_attr( $home_link ) . '" class="btTextLogo">' . get_bloginfo( 'name' ) . '</a>';
		}
	}
}
...
if ( ! function_exists( 'boldthemes_pagination' ) ) {
	function boldthemes_pagination() {
		...
		$prev = get_previous_posts_link( esc_html__( 'Newer Posts', 'squadrone' ) );
		$next = get_next_posts_link( esc_html__( 'Older Posts', 'squadrone' ) );
...
if ( ! function_exists( 'boldthemes_theme_comment' ) ) {
	function boldthemes_theme_comment( $comment, $args, $depth ) {
		...
		<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
			<p><?php esc_html_e( 'Pingback:', 'squadrone' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( esc_html__( '(Edit)', 'squadrone' ), '<span class="edit-link">', '</span>' ); ?></p>
		<?php
				break;
			default :
				...
				<div class="commentTxt">
					<div class="vcard divider">
						<?php
							...
							echo '<p class="posted">' . sprintf( esc_html__( '%1$s at %2$s', 'squadrone' ), get_comment_date(), get_comment_time() ) . '</p>';
							if ( $rating > 0 && get_option( 'woocommerce_enable_review_rating' ) == 'yes' ) { ?>
								<div itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating" class="star-rating" title="<?php echo sprintf( esc_html__( 'Rated %d out of 5', 'squadrone' ), $rating ) ?>">
									<span style="width:<?php echo wp_kses_post( ( $rating / 5 ) * 100 ); ?>%"><strong itemprop="ratingValue"><?php echo wp_kses_post( $rating ); ?></strong> <?php esc_html_e( 'out of 5', 'squadrone' ); ?></span>
								</div>
							<?php }
						?>
					</div>
					...
					<?php if ( '0' == $comment->comment_approved ) : ?>
						<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'squadrone' ); ?></p>
					<?php endif; ?>
					...
					<div class="comment">
						...
						if ( comments_open() ) {
							echo '<p class="reply">';
								comment_reply_link( array_merge( $args, array( 'reply_text' => esc_html__( 'Reply', 'squadrone' ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) );
							echo '</p>';
						}
						edit_comment_link( esc_html__( 'Edit', 'squadrone' ), '<p class="edit-link">', '</p>' ); ?>
					</div>
				</div>
			...
			if ( isset( BoldThemesFramework::$fake_customizer->control_arr[ $part_meta_key ] ) ) {
				if ( isset( BoldThemesFramework::$fake_customizer->control_arr[ $part_meta_key ]['type'] ) ) {
					...
					if ( $type == 'checkbox' ) {
						...
						if ( $src != '' ) {
							$input .= '<img src="' . esc_attr( $src ) . '"><span class="boldthemes_override_add_image"><i class="dashicons dashicons-plus-alt"></i></span>';
						} else {
...
if ( ! function_exists( 'boldthemes_get_posts_array_item' ) ) {
	function boldthemes_get_posts_array_item( $post_type = 'blog', $post_id, $post_author ) {
		...
		if ( $user_data ) {
			...
			$post_data['author'] = '<a href="' . esc_attr( $author_url ) . '" class="btArticleAuthorURL">' . esc_html( $author ) . '</a>';
...
if ( ! function_exists( 'boldthemes_set_global_uri' ) ) {
	function boldthemes_set_global_uri() {
		...
		$data .= 'window.boldthemes_text.previous = \'' . esc_html__( 'previous', 'squadrone' ) . '\';';
		$data .= 'window.boldthemes_text.next = \'' . esc_html__( 'next', 'squadrone' ) . '\';';
		return $data;
	}
}
...
if ( ! function_exists( 'boldthemes_get_post_author' ) ) {
	function boldthemes_get_post_author( $author_url = false, $prefix = 'by', $show_avatar = false ) {
		...
		$output .= '<a href="' . esc_attr( $author_url ) . '" class="btArticleAuthorURL">' . $prefix . ' ' . esc_html( get_the_author_meta( 'display_name', $post_author_id )  ) . '</a>';
		$output .= '</span>';
		return $output;
	}
}
...
if ( ! function_exists( 'boldthemes_get_post_comments' ) ) {
	function boldthemes_get_post_comments() {
		return '<a href="' . esc_attr( get_permalink() ) . '#comments" class="btArticleComments">' . get_comments_number() . '</a>';
	}
}
	...
	function boldthemes_get_post_categories( $arg = array() ) {
		...
		if ( $categories ) {
			...
			foreach ( $categories as $cat ) {
				if ( ! $no_link ) $categories_html .= '<a href="' . esc_attr( get_term_link( $cat->term_id ) ) . '" class="btArticleCategory ' . $cat->slug . '">';
		...
		if ( ! is_404() && ! is_front_page() ) {
			...
			if ( ! $simple ) {
				...
				if ( ! is_singular() || is_page() ) {
					$output .= '<span><a href="' . esc_attr( $home_link ) . '">' . esc_html__( 'Home', 'squadrone' ) . '</a></span>';
				}
			} else {
				if ( ! is_singular() || is_page() ) {
					$output .= '<a href="' . esc_attr( $home_link ) . '">' . esc_html__( 'Home', 'squadrone' ) . '</a>';
				}
			}
			...
			if ( is_home() ) {
				...
				foreach( $ancestors as $ancestor ) {
					$output .= wp_kses_post( $item_prefix ) . '<a href="' . esc_attr( get_permalink( $ancestor ) ) . '">' . wp_kses_post( get_the_title( $ancestor ) ) . '</a>' . wp_kses_post( $item_suffix );
				}
				...
				$output .= $item_prefix . esc_html__( 'Portfolio', 'squadrone' ) . $item_suffix;
				...
				$output .= $item_prefix . esc_html__( 'Category', 'squadrone' ) . $item_suffix;
				...
				$output .= $item_prefix . esc_html__( 'Tag', 'squadrone' ) . $item_suffix;
				
				$subtitle = '';
		  
			} else if ( is_author() ) {
			
				$output .= $item_prefix . esc_html__( 'Author', 'squadrone' ) . $item_suffix;
				...
				$output .= $item_prefix . esc_html__( 'Search', 'squadrone' ) . $item_suffix;
